const priceEA = {
    현수막: {
        수성: {
            헤베: 3000, //장폭헤베가격
            // [0:미터당추가, 1:1미터, 2: 2미터, 3: 3미터, 4: 4미터, 5: 5미터]
            폭30: {
                기본가: 2000,
                미터1: 4000,
                미터2: 6000,
...
function Price(inkType, paperType, imageWidth, imageLength, coUnt, calCulate, metHod, printWidth) {
...
function Aaa(tyPe) {
    ...
    //치수입력
    const imageWidth = document.getElementById('widthprint').value;
    const imageLength = document.getElementById('heightprint').value;
    const coUnt = document.getElementById('eaprint').value;
    
    //원단종류
    let pP = document.getElementById('papertype');
    let pPIndex = document.getElementById('papertype').options.selectedIndex;
    let paperType = pP.options[pPIndex].value;
